...
async function generateCertificatePDFBuffer(meta) {
  ...
  page.drawText(brand, {
    x: 48, y: height - 86, size: 28, font: helvBold, color: rgb(1, 1, 1)
  });

  page.drawText(`${country} • ${mode}`, {
    x: 48, y: height - 118, size: 13, font: helv, color: rgb(0.8, 0.82, 0.85)
  });

  page.drawText(`${lat}, ${lon}`, {
    x: 48, y: height - 180, size: 22, font: courierBold, color: rgb(1, 1, 1)
  });

  page.drawText(`Tile: ${tileM} m × ${tileM} m (${areaM2} m²)`, {
    x: 48, y: height - 214, size: 14, font: helv, color: rgb(0.9, 0.9, 0.95)
  });

  page.drawText(`Seed: ${seed}`, {
    x: 48, y: height - 242, size: 11, font: courier, color: rgb(0.75, 0.78, 0.82)
  });
  ...
  for (const line of legalLines) {
    page.drawText(line, { x: 48, y, size: 11, font: helv, color: rgb(0.70, 0.72, 0.76) });
    y -= 18;
  }

  page.drawText(`Issued: ${issued}`, {
    x: 48, y: 44, size: 10, font: helv, color: rgb(0.60, 0.62, 0.65)
  });
...
exports.handler = async (event) => {
  ...
  try {
    // ---------- CREATE CHECKOUT ----------
    if (action === "create") {
      if (!STRIPE_PRICE_ID || !SITE_URL) return json(500, { error: "Missing STRIPE_PRICE_ID or SITE_URL" });
      ...
      const email_consent = asStr(body.email_consent || "no", 10).toLowerCase() === "yes" ? "yes" : "no";
    ...
    if (action === "verify") {
      ...
      if (!session_id || !session_id.startsWith("cs_")) return json(400, { error: "Missing/invalid session_id" });
    ...
    if (action === "deliver") {
      ...
      if (!consentYes) return json(200, { ok: true, delivered: false, message: "Email delivery not opted in." });
      if (!to || !looksLikeEmail(to)) return json(200, { ok: false, delivered: false, message: "Missing/invalid email." });

      const pi = session.payment_intent;
      if (!pi || typeof pi !== "object" || !pi.id) return json(500, { ok: false, delivered: false, message: "Missing payment_intent." });
      ...
      const filename = `certificate-${String(meta.country || "XX").replace(/\s+/g, "_")}-${String(meta.seed || "seed").slice(0, 10)}.pdf`;
    ...
    return json(400, { error: "Unknown action. Use ?action=create | verify | deliver" });
